...
const selectedColor = ref(product.colors[2])
      ...
      <div class="mt-4 lg:row-span-3 lg:mt-0">
        ...
        <div class="mt-6">
          ...
          <div class="flex items-center">
            <div class="flex items-center">
              <Icon
                v-for="rating in [0, 1, 2, 3, 4]" :key="rating" name="ic:round-star" class="h-5 w-5 flex-shrink-0"
                :class="[reviews.average > rating ? 'text-gray-900' : 'text-gray-200']" aria-hidden="true"
              />
            </div>
            <p class="sr-only">
        ...
        <div class="mt-10">
          ...
          <div>
            ...
            <HeadlessRadioGroup v-model="selectedColor" class="mt-4">
              ...
              <div class="flex items-center space-x-3">
                ...
                >
                  <div
                    ...
                    :class="[color.selectedClass, active && checked ? 'ring ring-offset-1' : '', !active && checked ? 'ring-2' : '']"
                  >
                    <HeadlessRadioGroupLabel as="span" class="sr-only">
                      {{ color.name }}
                    </HeadlessRadioGroupLabel>
                    <span
                      aria-hidden="true" class="h-8 w-8 rounded-full border border-black border-opacity-10"
                      :class="[color.class]"
                    />
                  </div>
                </HeadlessRadioGroupOption>
          ...
          <div class="mt-10">
            ...
            <HeadlessRadioGroup v-model="selectedSize" class="mt-4">
              <HeadlessRadioGroupLabel class="sr-only">
                Choose a size
              </HeadlessRadioGroupLabel>
              <div class="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                <HeadlessRadioGroupOption
                  v-for="size in product.sizes" :key="size.name" v-slot="{ active, checked }"
                  as="template" :value="size" :disabled="!size.inStock"
                >
                  <div
                    ...
                    :class="[size.inStock ? 'cursor-pointer bg-white text-gray-900 shadow-sm' : 'cursor-not-allowed bg-gray-50 text-gray-200', active ? 'ring-2 ring-indigo-500' : '']"
                  >
                    <HeadlessRadioGroupLabel as="span">
                      {{ size.name }}
                    </HeadlessRadioGroupLabel>
                    <span
                      v-if="size.inStock" class="pointer-events-none absolute -inset-px rounded-md"
